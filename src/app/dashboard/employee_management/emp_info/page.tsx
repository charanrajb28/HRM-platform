...
import { employees as emp_data, departments } from "@/app/data/dummy";
        ...
        <tbody>
          {emp_data.map((emp) => (
            <tr key={emp.id} className=" hover:bg-gray-100 py-7">
              ...
              <td className="px-4 py-2">{departments.find(d => d.id === emp.departmentId)?.name || 'N/A'}</td>
              <td className="px-4 py-2">{emp.authority}</td>

              <td className="px-6 py-4">
                <div className="flex gap-7 items-center text-gray-600">
                  <Link href={`/dashboard/employee_management/emp_info/show/${emp.id}`}>
